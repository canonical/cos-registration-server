...
def device(request, uid):
    ...
    foxglove = base_url + '/cos-foxglove-studio/?ds=foxglove-websocket&ds.url=ws%3A%2F%2F' + device.address + '%3A8765/'
    bag_files = base_url + '/cos-ros2bag-fileserver/' + uid + '/'
    links = {'grafana folder': grafana_folder, 'foxglove': foxglove, 'bag_files': bag_files}
    context = {
        "device": device,
        "links_dict": links,
    }
